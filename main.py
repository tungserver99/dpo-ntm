...
if __name__ == "__main__":
    ...
    use_update = args.enable_update and args.model == "ECRTM"
    ...
    # save beta, theta and top words
        if use_update and base_content_dir and not args.update_only:
            ...
            base_snapshot_dir = args.update_dir if args.update_only else base_content_dir
            base_snapshot_path = os.path.join(
                base_snapshot_dir, f"update_snapshot_epoch_{args.update_start_epoch}.pth"
            )
    ...
    evaluate(args, trainer, dataset, train_theta, test_theta, current_run_dir, logger, read_labels=True, eval_tag=final_tag)
